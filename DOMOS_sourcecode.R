...
gam.t2=gam(sqrt(AirtimeCoeff)~
            # gamt_1 low impact.. from lasso
          ClaCount.CngestdWifi
          +s(ClaCount.CngestdNonWifi)
          +Samples
          +s(activeSamples, k=15)
          +s(lnCSA.badtx, k=15)
          ...
          # gamt_2 bigger impact
          +s(lnRxBytes, k=15)+s(lnTxBytes)+s(lnRxRe_sad)+s(lnTxRe_sad, k=15)
          +s(lnCSA.tx)+s(lnCSA.inbss, k=15)
          # gamt_3 related to interference
          +s(NumCompAPsSameChnl, k=10)
          +s(NumNonCompAPsOthrChnls, k=13)
          +s(ChaminStatsAverage.knoise, k=15)
          +s(refl_lnCSA.txop, k=10)
          +s(in_network_SumAllDevices, k=15)
          +s(lnCSA.obss, k=15)
          # 
          +s(lnCSA.nopkt, k=15)
          +s(ClaCount.PktLoss, k=15)
          ...
          +s(lnRxDecryptFail_sad)
          +s(lnTxPktsReExhaust_sad, k=10)
          , data=train.t, family = gaussian(), select = TRUE)

par(mfrow=c(3,3))
plot.gam(gam.t2, se=T, scale=0, col="blue")
summary(gam.t2)
summary.aov(gam.t2)
gam.check(gam.t2, cex=0.1)

# RMSE and MAE
y.hat.test.gam.t2 <- (predict.gam(gam.t2, newdata = test.t))^2

RMSE.gam.t2 <- sqrt(mean((y.hat.test.gam.t2-test.t$AirtimeCoeff)^2))
RMSE.gam.t2
[1] 0.1594064

MAE.gam.t2 <- mean(abs(y.hat.test.gam.t2-test.t$AirtimeCoeff))
MAE.gam.t2
[1] 0.1131857
